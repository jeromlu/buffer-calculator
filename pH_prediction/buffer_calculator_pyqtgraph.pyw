...
LEFT, ABOVE = range(2)
ACID_NAMES = ['Acetate', 'Phosphate','Succinate','Tris','Citrate','Malate','Arginine',
              'Histidine','Cysteine','EDTA','NaCl']
...
class BufferCalculatorUI(QMainWindow):
    ...
    def create_main_window(self):
        ...
        #alternative_with pyqtgraph
        self.plot_widget = pg.PlotWidget()
        self.plot_widget.setWindowTitle('pyqtgraph example: MultiplePlotAxes')
        self.p_pH = self.plot_widget.plotItem
        
        ## create a new ViewBox, link the right axis to its coordinate system
        self.p_cond = pg.ViewBox()
        self.p_pH.showAxis('right')
        self.p_pH.scene().addItem(self.p_cond)
        self.p_pH.getAxis('right').linkToView(self.p_cond)
        self.p_cond.setXLink(self.p_pH)
        self.p_pH.getAxis('left').setLabel('pH', color = COLORS[0],
                         **{'font-size' : '14pt'})
        self.p_pH.getAxis('right').setLabel('Conductivity [mS/cm]', color=COLORS[1],
                          **{'font-size' : '14pt'})
        self.p_pH.getAxis('bottom').setLabel('Temperature [C]',
                         **{'font-size' : '14pt'})

        plotDatapH = self.p_pH.plot([1,2,4,8,16,32], 
                       pen=pg.mkPen(width=3, color = COLORS[0]),
                       name = "pH")
        plotDataCond = pg.PlotCurveItem([10,20,40,80,40,20],
                                             pen=pg.mkPen(width=3,color=COLORS[1]),
                                              name="Conductivity")
        
        self.p_cond.addItem(plotDataCond)
        #self.p_pH.showGrid(x=True, y=True)
        legend = pg.LegendItem(offset = (100, 1))
        legend.setParentItem(self.plot_widget.graphicsItem())
        legend.layout.setHorizontalSpacing(20)
        legend.addItem(plotDatapH,"pH")
        legend.addItem(plotDataCond,"Cond")

        ## Handle view resizing 
        ## view has resized; update auxiliary views to match
        self.p_cond.setGeometry(self.p_pH.vb.sceneBoundingRect())
        
        ## need to re-update linked axes since this was called
        ## incorrectly while views had different shapes.
        ## (probably this should be handled in ViewBox.resizeEvent)
        self.p_cond.linkedViewChanged(self.p_pH.vb, self.p_cond.XAxis)    
        ...
        self.tab_temperature.layout.addWidget(self.plot_widget)
    ...
    def create_menu_bar(self):
        ...
        file_quit_action = self.create_action("&Close app",
            shortcut="Ctrl+Q", slot = QApplication.quit, 
    ...
    def calculate_buff_composition(self):
        ...
        if out_array[-1] == '':
            QMessageBox.warning(self, 'Volume error','If you are preparing buffer of no volume,'\
                                'then you are already finished.\n\nCongratiolations!! :)')
            return
        #print(out_array)
        communicator = pH_calculation_Rushd.FortranCommunication(folder = application_path + '/testing/')
        communicator.write_parameters(out_array)
        #print(self.composition)
        communicator.run_subprocess()
        if not communicator.read_data(self):
            QMessageBox.critical(self, 'Error', 'Check your values,\n'\
    ...
    def update_temp_dep(self):
        ...
        try:
            ...
            if self.plot_lns[0]:
                ...
                self.ax_pH.set_ylim(target_pH-2, target_pH+2)
                self.ax_cond.set_ylim(min(y_cond), max(y_cond))
                self.plot_lns[0], = self.ax_pH.plot(x, y_pH, 
                             label = 'Buffer pH', 
                             color = COLOR_PH, linewidth = 2 )
                
                self.plot_lns[1], = self.ax_cond.plot(x, y_cond, 
                             label = 'Buffer conductivity',
                             color = COLOR_COND, linewidth=2)
...
def excepthook(excType, excValue, tracebackobj):
    ...
    notice = \
        ...
        .format("luka.jeromel@novartis.com", "")
    versionInfo="0.0.1"
